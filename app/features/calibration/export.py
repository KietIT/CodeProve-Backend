...
anchored). Session ids are derived from a random salt that is never stored,
...
async def _session(db: AsyncSession, sid: str, attempt: Attempt, ex: Exercise) -> dict | None:
    ...
    return {
        ...
        "duration_min": _minutes(events[-1]["ts"], start),
        "hypotheses": [
...
async def build_sessions(db: AsyncSession, limit: int = 60, per_user: int = 3,
                         salt: str | None = None) -> tuple[list[dict], dict]:
...
async def _main(out: Path, limit: int, per_user: int) -> None:
    async with async_session_maker() as db:
        sessions, engine = await build_sessions(db, limit, per_user)
...
if __name__ == "__main__":
    ...
    asyncio.run(_main(args.out, args.limit, args.per_user))
